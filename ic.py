...
<div style='text-align:center'>
    <h1 style='color:#008000;'>INDUSTRIAL COPPER MODEL
    </h1>
</div>
...
with tab2:
    with st.form("my_form1"):
        ...
        with col1:
            cquantity_tons = st.text_input("Quantity Tons")
        ...
        with col3:
            ...
            csubmit_button = st.form_submit_button(label="PREDICT")
    ...
    if csubmit_button and cflag == 0:
        ...
        #new_sample_ohe = t_loaded.transform(new_sample[:, [7]])
        #new_sample_scaled = scaler_loaded.transform(
         #   np.concatenate((new_sample[:, :7], new_sample_ohe), axis=1)
        #)

        new_sample = cscaler_loaded.transform(new_sample)
        new_pred = cloaded_model.predict(new_sample)
        if new_pred == 1:
            st.write('## :red[The Status is Won] ')
        else:
            st.write('## :red[The status is Lost] ')

st.write(f'<h6 align=center style="color:rgb(0, 153, 153,0.35);">@Industrial Copper EDA</h6>', unsafe_allow_html=True)